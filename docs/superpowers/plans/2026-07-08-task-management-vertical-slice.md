...
pub fn set_tasks_config(
    ...
    let folder = tasks_folder
        ...
        .filter(|f| !f.is_empty())
        .map(str::to_string);
    // Validate the folder that will ACTUALLY be used — explicit, or the default
    // "Tasks" when cleared — against a symlinked ancestor (even a missing leaf),
    // so clearing to a symlinked default is rejected up front too, not just
    // custom folders. ("Tasks" mirrors VaultCaptureConfig::tasks_root()'s default.)
...
pub fn add_task(id: String, title: String) -> Result<TaskDto, String> {
    ...
    // A stale registry can list a vault whose folder was moved/deleted; without
    // this guard create_dir_all would resurrect it and write into a non-vault.
    ...
    // ancestor is caught even when the leaf doesn't exist yet, and create_dir_all
    // can't create a directory (or write a task) outside the vault.
