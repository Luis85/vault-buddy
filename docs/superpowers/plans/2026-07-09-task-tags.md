...
- The grouping toggle, inserted as a sibling after the add-options row and BEFORE the loading/error/empty `v-if` chain (it must not sit between the chain's `v-if`/`v-else-if`/`v-else` siblings — that severs Vue's adjacency and collapses the list):
