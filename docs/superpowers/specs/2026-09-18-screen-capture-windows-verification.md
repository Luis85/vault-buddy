...
| 43 | **Vault Buddy must not break other applications' input** (GAP-166) | With Vault Buddy running, use another app — File Explorer especially — and **try its toolbar and menus while a Vault Buddy window has focus**. **Record:** which application, which control, whether the FIRST click is swallowed and a second works, whether the Vault Buddy panel was open at the time, whether a screen capture had been run since launch, and whether tray → **Quit** restores the behaviour. This row exists because a companion app that quietly disables another program's UI is worse than any defect inside our own surfaces, and it is invisible from inside our app. | **2026-09-21: FAILED.** File Explorer's top toolbar stopped accepting clicks while the editor or the buddy was the active window — the WHOLE toolbar, not a patch — and tray → Quit restored it. Buddy parked bottom-right, so it is not the buddy's 88x88 box; the editor is `alwaysOnTop: false`, so no topmost window of ours need be over Explorer at all. **Unlocalised** — see GAP-166 for what is established and the three discriminators still outstanding. |
