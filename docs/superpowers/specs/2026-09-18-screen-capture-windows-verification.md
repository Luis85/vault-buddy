...
| 4 | **Pause excludes its stretch** (spec §6.2/6.3) | Record ~20 s with a visible timer on screen, Pause from the tray, wait ~20 s (change the screen visibly during the pause), Resume, record ~20 s more, Stop. | **2026-09-19: PARTIAL.** Pause and Resume confirmed working as controls. The measurement this row exists for is still open: file duration (~40 s vs ~60 s), whether the paused stretch is absent, and A/V sync after resume. Those test the shared `CaptureClock`, which can be wrong in the file while the UI looks correct — do not mark this row done on the controls alone. |
