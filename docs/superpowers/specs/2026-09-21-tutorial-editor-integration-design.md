...
| `editor_relink_media` | async | `sessionId, assetIds: string[]` | `RelinkReport \| null` |
| `editor_media_url` | async | `sessionId, ref: { assetId } \| { productId }` | `string` (absolute path for `convertFileSrc`) |
