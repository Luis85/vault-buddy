...
## Retiring the phase-4 editor and the phase-5 export (Task 59)

Task 59 deleted the capture editor's own Save (the phase-5 export) and its
Discard. "Save unchanged" is now **Render video** (an untouched capture is
R1's identity plan: a lossless `-c copy` remux) plus **Publish to vault…**,
and throwing a recording away is **Discard project…** (which unpins it)
followed by the Record Screen list's **Discard**.
`identity_render_of_an_untouched_capture_is_lossless`
(`screen/tests/render_roundtrip.rs`) proves the remux copies the encoded
video verbatim against a synthesized clip, and `tests/editorRoot.test.ts`
drives the discard dialog over a fake port; neither touches a real
Media Foundation capture, a real vault or Windows' file locking. These rows
do, and replace the screen-capture checklist's rows 29, 31, 34 (editor
half) and 38.
