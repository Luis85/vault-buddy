...
//! code they pin (the `services::tasks::tests` precedent). The concurrency/
//! race regression tests live in the sibling `concurrency` module (this
//! file grew past the LOC cap again once they landed; see that module's
//! own doc comment for why they were split out as a themed group rather
//! than an arbitrary tail cut).
