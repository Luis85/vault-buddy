...
fn transition_side_reused_is_rejected() {
    ...
    // so it ends at 1000ms. c2 and c3 both start at 900ms on the same
    // track -- exactly the 100ms overlap a 100ms transition creates (Task
    // 30) -- so a transition from c1 into EITHER is individually valid —
    // but c1 cannot be the "from" side of two transitions at once.
    let mut project = base_project();
    project.clips.push(
        serde_json::from_value(serde_json::json!({
            "id": "c2", "asset_id": "a1", "track_id": "t1", "name": "Clip Two",
            "start_ms": 900, "in_ms": 0, "out_ms": 1000,
            "fade_in_ms": 0, "fade_out_ms": 0, "fade_curve": "linear",
            "opacity": 1, "volume": 1, "muted": false,
            "x": 0, "y": 0, "w": 1, "h": 1
        }))
    ...
    project.clips.push(
        serde_json::from_value(serde_json::json!({
            ...
            "start_ms": 900, "in_ms": 0, "out_ms": 500,
            "fade_in_ms": 0, "fade_out_ms": 0, "fade_curve": "linear",
            "opacity": 1, "volume": 1, "muted": false,
            "x": 0, "y": 0, "w": 1, "h": 1
        }))
...
fn transition_to_side_reused_is_rejected() {
    ...
    // clips (c1 and c5) each end exactly at 1000ms, so a 100ms transition
    // from EITHER into c6 (which starts at 900ms, the overlap such a
    // transition creates) is individually valid — but c6 cannot be the
    // "to" side of two transitions at once.
    let mut project = base_project();
    project.clips.push(
        serde_json::from_value(serde_json::json!({
