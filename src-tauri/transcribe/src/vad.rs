...
/// covers it; ties at a span-to-span boundary resolve to the EARLIER
/// entry's original end (checked first, via `<=`) rather than the next
/// entry's original start — whisper can legitimately emit a segment's end
/// timestamp exactly there, and the earlier entry's end is the closer
/// original-timeline instant (the two can differ by an entire collapsed
/// silence gap). A position past every entry (whisper emitting a
/// timestamp at, or a hair past, the filtered buffer's very end) clamps to
/// the last entry's original end. An empty map (defensive — should not
/// happen once VAD produced any segment) returns `filtered_ms` unchanged:
/// the safest identity when there is nothing to map through.
pub fn remap_ms(filtered_ms: u64, map: &[SpanMap]) -> u64 {
...
mod tests {
    ...
    fn identity_when_one_span_covers_everything() {
        ...
        for ms in [0, 1, 250, 500, 999] {
            assert_eq!(remap_ms(ms, &map), ms, "identity at {ms}ms");
        }
    }
    ...
    fn filtered_timestamp_inside_span_two_maps_to_original_span_two() {
        ...
        // contribution -> original 2000 + 500 = 2500ms.
        let spans = vec![
            SpeechSpan {
                start: 0,
                end: 16_000,
        ...
        assert_eq!(remap_ms(1500, &map), 2500);
    }

    #[test]
    fn gap_boundary_clamps_to_the_preceding_spans_original_end() {
        ...
        // filtered timeline (they're concatenated back-to-back) but must
        // resolve to span1's ORIGINAL end (1000ms), not span2's original
        // start (2000ms) — whisper emitting a segment end exactly here
        // means "speech stopped at the end of span1", not "speech resumed
        // at span2". The two differ by the entire collapsed silence gap.
        let spans = vec![
            SpeechSpan {
                start: 0,
                end: 16_000,
        ...
        assert_eq!(remap_ms(1000, &map), 1000);
    }
    ...
    fn past_the_last_span_clamps_to_its_original_end() {
        ...
        // the last span's original end, not extrapolate past it.
        ...
        assert_eq!(
            remap_ms(5_000, &map),
            1_000,
            "5s is past the 1s filtered buffer"
        );
    }
    ...
    fn remap_with_an_empty_map_returns_the_input_unchanged() {
        ...
        // sanely rather than panic or invent a value.
        assert_eq!(remap_ms(0, &[]), 0);
        assert_eq!(remap_ms(1_234, &[]), 1_234);
    }
}
