...
import { useTaskDetailSubtasks } from "../composables/useTaskDetailSubtasks";
...
// The Subtasks section's verbs (create / toggle / drill in), on the same
// shared `busy` guard as every other write here — see useTaskDetailSubtasks.
const { addSubtaskDisabledReason, onAddSubtask, onToggleSubtask, openSubtaskDetail } = useTaskDetailSubtasks({
  task: taskRef,
  busy,
  allTasks,
  reloadTaskSet,
  archivedLists,
  archivedListsResolved,
  archivedListsError,
  resetInput: () => subtasksRef.value?.reset(),
});
</script>
