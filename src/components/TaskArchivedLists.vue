...
<template>
  <template v-if="lists.length > 0">
    <p class="mb-1 mt-2 text-sm text-slate-200">
      Archived lists
      <span class="block text-xs text-slate-500">Hidden from the tasks view; unarchive to bring one back</span>
    </p>
    <ul class="flex flex-col gap-1">
      ...
      >
        <span class="min-w-0 flex-1 truncate text-sm text-slate-400">{{ list }}</span>
        <button
          type="button"
          :data-testid="`unarchive-${list}`"
          :aria-label="`Unarchive ${list}`"
          class="cursor-pointer rounded px-1.5 py-0.5 text-xs text-slate-300 transition-colors hover:bg-white/10 hover:text-slate-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-violet-400"
