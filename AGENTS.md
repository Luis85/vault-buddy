...
All 118 commands, registered in `src-tauri/src/lib.rs` (`generate_handler`).
...
`editor_import_package`; 118 after Task 40's `editor_relink_media`):
...
3. A grant in EXACTLY ONE capability file's `permissions` array:
   ...
   command is one of the `editor_*` commands (twenty as of Task 40), otherwise
