...
│   │                           #   16 editor_* ones) + editor.json (those 16, the
...
All 112 commands, registered in `src-tauri/src/lib.rs` (`generate_handler`).
...
3. A grant in EXACTLY ONE capability file's `permissions` array:
   ...
   command is one of the `editor_*` commands (sixteen as of Task 28), otherwise
