...
│   │                           #   src/editor/guide/{position,prepare}.ts
...
│   │                           #   32 editor_* ones) + editor.json (those 32, the
...
│   │                           #     revision or history; since Task 55 also the app-wide
│   │                           #     editor-prefs\guide-progress.json read/write) + the media surface
...
3. A grant in EXACTLY ONE capability file's `permissions` array:
   ...
   command is one of the `editor_*` commands (thirty-two as of Task 55, measured in `editor.json`), otherwise
...
control is `markExplored` and never advances the lesson), Help and F1/?
