...
| `editor/caption_commands.rs` | Subtitle import onto one clip (Task 36; F-34; ADR §3.3), its own module because `media_commands.rs` sits at the 800-line cap. `editor_import_captions` *(async — `sessionId`, `clipId`, `replace` → `CaptionImportResult { projection, imported, skipped }`, or `null` when the dialog was cancelled; the session is checked BEFORE the dialog opens)* opens its OWN native open dialog (`srt`, `vtt`, `txt`) on a named `editor-captions` thread joined from the blocking pool — the dialog, never a frontend string, grants the path (the Task 25 posture). The file is read BOUNDED (at most `MAX_CAPTION_FILE_BYTES` + 1 bytes, UTF-8 or refused), parsed by `core::editor::captions_io` (SRT + plain WebVTT; `STYLE`/`REGION`/`NOTE` skipped, tags stripped; ≤ 2 MiB, ≤ 2000 cues, ≤ 500 characters a cue, ≤ 2 h; every error names its 1-based LINE), then `core::editor::commands::captions::plan_caption_import` maps the file's times — OUTPUT time relative to the clip's own start — to the clip's SOURCE time through the inverse of `time::output_at` (`in_ms + round(relative · speed)`); a cue starting at or past the clip's output end is SKIPPED and counted, one running past it is cut at the clip's source end, and a file with nothing inside the clip is refused (`invalidRequest`) rather than applied as an empty — with `replace`, destructive — edit. The whole import is ONE `InternalCommand::ImportCaptions` edit (one Undo). No error or log line carries the picked file's name, path or text (bundle § Privacy): a failed open/read logs only the `io::ErrorKind`. `captions_io::export_srt`/`export_vtt` (cues already in OUTPUT time) are Task 48's `editor_export_subtitles`' writers. Same NATIVE `require_editor_window` + `authz_guard.rs` discipline as its siblings. |
