...
│   │                           #   34 editor_* ones) + editor.json (those 34, the
...
All 133 commands, registered in `src-tauri/src/lib.rs` (`generate_handler`).
...
3. A grant in EXACTLY ONE capability file's `permissions` array:
   ...
   command is one of the `editor_*` commands (thirty-four as of Task 57, measured in `editor.json`), otherwise
