...
All 121 commands, registered in `src-tauri/src/lib.rs` (`generate_handler`).
...
- The metronome thread heartbeat-refreshes a run marker (`app_diagnostics`
  ...
  **There are FOUR window-label lists, and they are four on purpose.** The
  ...
  is killed and its part deleted), then finalizes the audio recording and
  the screen capture, then `finish_quit`. `close-finalize` (Alt+F4) runs the
  same two cancels in the same order; a render that outlives its bound is
  logged and stops counting in the gate, so the re-triggered close cannot
  loop. Cancel rather than finalize because an export is
...
logged (`uri::launch`) as the audit trail. Nine sanctioned write paths
...
live in that crate. Any other code touching vault contents
