...
│   │                           #   17 editor_* ones) + editor.json (those 17, the
...
All 128 commands, registered in `src-tauri/src/lib.rs` (`generate_handler`).
...
after Task 52's `list_capture_webcams`):
...
3. A grant in EXACTLY ONE capability file's `permissions` array:
   ...
   command is one of the `editor_*` commands (twenty-three as of Task 46), otherwise
...
an acknowledged edit like `execute`), and
