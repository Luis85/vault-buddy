...
│   │                           #   14 editor_* ones) + editor.json (those 14, the
...
│   │                           #     import's own ffprobe rule) + their caller-window scan),
...
All 110 commands, registered in `src-tauri/src/lib.rs` (`generate_handler`).
...
not incremented (110 after Task 25's `editor_import_media`/`editor_cancel_job`/
`editor_get_jobs`):
...
3. A grant in EXACTLY ONE capability file's `permissions` array:
   ...
   command is one of the `editor_*` commands (fourteen as of Task 25), otherwise
