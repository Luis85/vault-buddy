...
│   │                           #   5 editor_* ones) + editor.json (those 5, the
...
│   │                           #     editor_* session commands + their caller-window scan),
...
All 101 commands, registered in `src-tauri/src/lib.rs` (`generate_handler`).
...
3. A grant in EXACTLY ONE capability file's `permissions` array:
   ...
   command is one of the five `editor_*` session commands, otherwise
