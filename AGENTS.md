...
│   │                           #   30 editor_* ones) + editor.json (those 30, the
...
│   │                           #     revision or history) + the media surface
...
All 129 commands, registered in `src-tauri/src/lib.rs` (`generate_handler`).
...
3. A grant in EXACTLY ONE capability file's `permissions` array:
   ...
   command is one of the `editor_*` commands (thirty as of Task 54, measured in `editor.json`), otherwise
...
found"), and `notifications` (the toast queue rendered by `NotificationHost`).
