...
│   │                           #     commands (save_commands.rs) + their caller-window scan),
...
All 104 commands, registered in `src-tauri/src/lib.rs` (`generate_handler`).
...
| Tutorial editor projects | `%LOCALAPPDATA%\com.vaultbuddy.desktop\editor-projects\<projectId>\` — `project.json` (the workspace envelope) + `sources.json` (asset id → where its bytes live; a staged capture is referenced as `{"store":"staging","base":…}`, ADOPTED, never moved), later `products.json`/`recovery.json`/`workspace.json` and `media\`/`takes\`/`products\`/`cache\`/`jobs\`. **Outside every vault**, like staging. The link back is the PIN: `editorProjectId` in the staged capture's sidecar, written only AFTER the project exists (a crash in between leaves an unpinned orphan the next `editor_open_staged` adopts back by scanning `sources.json`). Removal is `store_io::remove_project` only — ownership proven, no-follow, never `remove_dir_all` |
